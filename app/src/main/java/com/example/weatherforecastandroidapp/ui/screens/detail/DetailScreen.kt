...
fun DetailScreenContent(
    ...
    if (showDeleteDialog) {
        AlertDialog(
            ...
            title = { Text(stringResource(Strings.delete_place_dialog_title)) },
            text = { Text(stringResource(Strings.delete_place_dialog_message, topBarText)) },
            confirmButton = {
                TextButton(onClick = {
                    showDeleteDialog = false
                    onDeleteClick()
                }) {
                    Text(stringResource(Strings.delete_place_dialog_confirm))
                }
            },
            dismissButton = {
                TextButton(onClick = { showDeleteDialog = false }) {
                    Text(stringResource(Strings.dialog_cancel))
                }
            },
        )
    }
